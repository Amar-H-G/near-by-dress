 ...
 * Purpose: shop business logic — CRUD, status management, Redis caching
 */
...
/** Invalidate all shop list cache keys */
...
const getApprovedShops = async (query) => {
  ...
  const [shops, total] = await Promise.all([
    Shop.find(filter).populate('owner', 'name email phone').sort({ createdAt: -1 }).skip(skip).limit(limit),
...
/** Public/Shared: get shop by ID with Redis cache */
const getShopById = async (id) => {
  ...
  if (redis) {
    try { await redis.setex(cacheKey, 600, JSON.stringify(shop)); } catch (_) {}
  }
...
/** Admin: list ALL shops with optional status/city filter */
...
/** Admin: update shop status (approve / reject) */
const updateShopStatus = async (shopId, { status, rejectionReason }) => {
  const shop = await Shop.findById(shopId);
  if (!shop) throw new AppError('Shop not found', 404);

  shop.status = status;
  shop.rejectionReason = status === 'rejected' && rejectionReason ? rejectionReason : null;
  await shop.save();

  await invalidateShopCache();
  const redis = getRedis();
  if (redis) { try { await redis.del(`shop:${shopId}`); } catch (_) {} }
...
/** Seller: create a new shop (one per owner) */
...
/** Seller/Admin: update shop details */
...
/** Seller: get the shop owned by this user */
